...
def BoyerMoore(original, pattern):
    if original is None or pattern is None or len(original) < len(pattern):
        return None
    ...
    if len2 == 1:
        ...
        while index < len1:
            ...
            while index < len1 and pattern[indexOfP] == original[index]:
                ...
                indexOfP -= 1
            if index < len1:
                index += max(goodTable[len2 - 1 - indexOfP],
                             badTable[original[index]])
...
def badCharTable(pattern):
    table = {}
    length = len(pattern)
    # 利用map生成一个原始移动表, 对应字符为从SPACE到~, 使用UTF-8对应表
    alphaBet = list(map(chr, range(32, 127)))
    for i in alphaBet:
        table[i] = length
